...
Generates:
    ...
    src/mn_config.s     – per-mnemonic config tables, mn7 7-bit hash (256 bytes)
    src/mn6_tables.s    – per-mnemonic config tables, mn6 6-bit hash (256 bytes)
"""
import functools
import operator
import pathlib
...
def write_config_table(out_path=None):
    """
    Write src/mn_config.s: two 128-byte tables indexed by 7-bit hash h7.

        mn_base_op[h]  base opcode for the mnemonic at hash slot h
        mn_profile[h]  operand profile index (0–29); bits 7:5 = 0

    256 bytes total.  Empty slots contain $00/$00.

    Retired: use mn7_tables.s directly. mn_config.s removed.
    """
    if out_path is None:
        out_path = pathlib.Path(__file__).parent / '../src/mn_config.s'
    out_path = pathlib.Path(out_path)
    ...
    for h, mne in slot_mne.items():
        ...
        if base_op is None:
            base_op = 0x00
        base_op_table[h] = base_op
        profile_table[h] = profile
    ...
    emit_table('mn_base_op', base_op_table)
    emit_table('mn_profile', profile_table)

    out_path.write_text('\n'.join(lines) + '\n')
    print(f"wrote {out_path.resolve()}  ({2 * 128} bytes, "
          f"{len(slot_mne)}/128 slots filled)")
...
def write_config_table_7bit(out_path=None):
    ...
    Write src/mn7_tables.s: three 128-byte tables indexed by 7-bit hash h7.
        ...
        mn7_base_op[h]   base opcode (same semantics as mn_config.s)
...
def write_config_table_6bit(out_path=None):
    ...
    Write src/mn6_tables.s: three 64-byte tables indexed by 6-bit hash h6.
        ...
        mn6_base_op[h]   base opcode (same semantics as mn_config.s)
...
if __name__ == '__main__':
    ...
    verify_base_opcodes()
    write_mode_table()
    # write_config_table()  ← REMOVED (mn_config.s retired)
    mn6.verify()
