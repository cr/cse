...
| Term | Definition |
|------|------------|
| **DDD System** | The totality of document-driven practices in this repository: the DDD Method, the TDD Method, the DDD Corpus, the DDD Maintenance and TDD Maintenance processes, and Escape Analysis.  The system's goal is that documentation is always the source of truth and never drifts from code.  Tests are the executable form of the contracts; they drift the same way prose does, and get audited the same way. |
...
| **TDD Maintenance** | A periodic audit of the test corpus, analogous to DDD Maintenance for prose.  Walks every exported symbol in every module doc, confirms each has a correctness test or a vocal skip, flags duplicates, implementation coupling, harness dependency-direction violations, and bundle-to-production parity gaps.  Also triggered reactively when an Escape Analysis amends a testing.md principle, to propagate that principle across the whole corpus.  See [README.md § TDD Maintenance](README.md#tdd-maintenance) for the full audit scope. |
...
| **Solitary unit testing** | Martin Fowler's term for tests that isolate the SUT with test doubles (mocks, stubs, fakes) for all collaborators.  Each test exercises exactly one module; dependencies are simulated.  Narrow blast radius, but only exercises the SUT against a *model* of its collaborators — so test passes don't prove integration.  See [Fowler 2014](https://martinfowler.com/bliki/UnitTest.html). |
| **Sociable unit testing** | Fowler's complement to solitary: tests drive the SUT with its *real* collaborators linked in.  A failing sociable test can implicate the SUT or any of its transitive dependencies; the blast radius is wider, but coverage is real code exercising real code.  Requires a codebase shape where each module's dependency closure is bounded and composable — which is what stratification delivers. |
| **Bundled unit testing** | CSE's flavor of sociable testing.  Each test bundle is a linker artifact built from the SUT plus its full dependency closure (no behavioural mocks — only minimal linker scaffolding in `dev/*_test_stub.s`).  One SUT per bundle; one test file per bundle; one production-build config mirrored per bundle (e.g. `asm_core` has cmos / 6510 / 6502 variants — see [testing.md § Principle 10](testing.md)).  Enabled by — and in turn enforces — the codebase's stratified shape: a bundle *is* a downward slice through the DAG. |
...
| **DDD Log** | Self-review of a session that examines how the DDD System served the work — where the Method, Corpus discipline, Maintenance, Escape Analysis, or test framework worked well, where they didn't, why, and concrete suggestions for improvement.  Distinct from the DDD Report (which summarises what changed) and from Escape Analysis (which reacts to a single escape): the Log evaluates the *system itself* against the lived experience of the session.  Findings feed the next Maintenance round; principle-level proposals feed testing.md / README.md amendments. |
| **Escape Analysis** | Reactive DDD process invoked when a bug is found that the test suite failed to catch.  Traces the escape backward: the missing test → the contract clause that should have prompted the test → the testing.md principle that should have forced the clause.  Amendments at all three layers are committed together, plus a two-axis corpus sweep: (a) class-wide (other code fitting the same bug shape) and (b) cross-module handoff (upstream callers whose state provision could silently defeat the new contract).  Each Escape Analysis tightens the corpus permanently — the same class of bug cannot escape twice.  See [README.md § Escape Analysis](README.md#escape-analysis). |
...
| **partial-mode function** | A function whose success depends on ancillary state — an input-pointer position, a residual-input marker, a progress counter — in addition to its return code.  Callers can only reason correctly about composition with a partial-mode function when that ancillary state is both documented (stopping-position table in the module doc) and executable (position-pinning tests per Principle 13).  Canonical example: `expr_eval` returns success with `expr_val` set AND leaves `expr_ptr` at the first unparsed byte; internal assembler callers rely on this to parse prefixes like `$10,X`, but single-expression REPL commands must enforce their own end-of-input check.  See [expr.md § Partial-mode contract](modules/expr.md#partial-mode-contract). |
