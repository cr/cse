...
| **Bundled unit testing** | CSE's flavor of sociable testing.  Each test bundle is a linker artifact built from the SUT plus its full dependency closure (no behavioural mocks — only minimal linker scaffolding in `dev/*_test_stub.s`).  One SUT per bundle; one test file per bundle; one production-build config mirrored per bundle (e.g. `asm_core` has cmos / 6510 / 6502 variants — see [testing.md § Principle 10](testing.md#principle-10-test-bundles-mirror-production-build-configs)).  Enabled by — and in turn enforces — the codebase's stratified shape: a bundle *is* a downward slice through the DAG. |
